...
>(({ className, sideOffset = 4, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        'z-50 min-w-[8rem] overflow-hidden rounded-md border border-slate-100 bg-white p-1 text-slate-700 shadow-md animate-in data-[side=bottom]:slide-in-from-top-2 data-[side=top]:slide-in-from-bottom-2 dark:border-slate-900 dark:bg-slate-950 dark:text-slate-400',
        className
      )}
      {...props}
    />
...
>(({ className, inset, ...props }, ref) => {
  const langauge = useLocalizationStore(state=>state.language)
  return <DropdownMenuPrimitive.Item
    ref={ref}
    dir={langauge=='ar'?'rtl':'ltr'}
    className={cn(
      'relative flex cursor-default select-none items-center rounded-sm py-1.5 px-2 text-sm font-medium outline-none focus:bg-slate-300 focus:cursor-pointer data-[disabled]:pointer-events-none data-[disabled]:opacity-50 dark:focus:bg-slate-800',
      inset && 'pl-8',
      className
    )}
    {...props}
