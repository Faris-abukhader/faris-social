...
>(({ className, ...props }, ref) => {
  ...
  >
    <SwitchPrimitives.Thumb
      className={cn(
        `pointer-events-none block h-5 w-5 rounded-full bg-white shadow-lg ring-0 transition-transform data-[state=unchecked]:translate-x-0 ${language=='ar'?'data-[state=checked]:-translate-x-5':'data-[state=checked]:translate-x-5'} `
      )}
    />
