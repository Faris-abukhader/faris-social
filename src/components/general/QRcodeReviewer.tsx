...
import { Popover, PopoverContent, PopoverTrigger } from '@faris/components/ui/popover'
...
export default function QRcodeReviewer({ target, path }: { target: Target, path: string }) {
        ...
        <Popover>
            ...
            <PopoverContent className=" w-56 p-2">
                ...
                <div className='w-full flex justify-center pt-2'>
                    <Button disabled={isLoading} onClick={() => void getImage()} size={'sm'} variant={'outline'}>{isLoading ? <Loading /> : <DownloadIcon className='w-4 h-4' />}</Button>
                </div>
            </PopoverContent>
        </Popover>
    )
