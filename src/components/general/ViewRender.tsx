...
export default function ViewRender<T>({
  ...
  if (isLoading)
    return (
      <div className={`${isGrid ? "grid gap-3 " : "space-y-3"} grid-cols-1 sm:grid-cols-2 md:grid-cols-3 py-3`}>
        {Array.from({ length: lengthOfSkeleton }).map((_, i) =>
          React.cloneElement(skeletonComonent, { key: i })
        )}
      </div>
    );
  ...
  if (data && data.length > 0) {
      ...
      <div>
        ...
        {hasNextPage && nextPage && (
          <div className="w-full flex justify-center pt-2">
            <Button onClick={nextPage} variant={"link"}>
              {t("seeMore")}
            </Button>
          </div>
        )}
    ...
    return (
      <IllustrationContainer
        ...
        className="mt-10"
        path={`/illustrations/${illustrations}.svg`}
        description={t(noDataMessage)}
      />
    );
  }
}
